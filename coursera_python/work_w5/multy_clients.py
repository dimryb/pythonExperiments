...
"""
import sys
from client2 import Client, ClientError
...
def run(host, port):
    ...
    try:
        client1._send(f"\n".encode())
        result = client1._read()
        if result != 'error\nwrong command\n\n':
            print(
                f"Ошибка произвольного запроса '\\n' {result}")

    except Exception as err:
        print(f"Ошибка произвольного запроса: "
              f"{err.__class__} {err}")
        sys.exit(1)

    print("test_9.1")
    try:
        client1._send(f"put palm.cpu aaa 1150864247\n".encode())
        result = client1._read()
        if result != 'error\nwrong command\n\n':
            print(
                f"Ошибка произвольного запроса '\\n' {result}")
